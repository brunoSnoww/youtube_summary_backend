...
use crate::yt_audio::error::{DEBUG_REPORT_PROMPT, ERROR_RETRY_PROMPT, UNRECOVERABLE_ERROR_PROMPT};

use super::assembling::youtube::config;
use super::error::{
    YtdlpError, ENCODER_STREAM_ERROR, HOMEPAGE_REDIRECT, NETWORK_FAIL, NONEXISTENT_PLAYLIST,
    NONEXISTENT_VIDEO, NO_API_PAGE, PRIVATE_VIDEO, REMOVED_VIDEO, SELECT_ALL, SELECT_NOTHING,
    VIDEO_NOT_FOUND, VIDEO_UNAVAILABLE, VIOLENT_VIDEO, YTDLP_GAVE_UP,
};
use super::parser;
        ...
        if !user_selection.is_empty() {
            if user_selection[0] == 0 {
                ...
                for video_to_re_download in &errors {
                    ...
                    to_be_downloaded.push(
                        download_config.build_command_for_video(video_to_re_download.video_id()),
                    );
                }
                ...
                for i in user_selection {
                    ...
                    to_be_downloaded.push(
                        download_config.build_command_for_video(errors[i - 2].video_id().as_str()),
                    );
                }
            }
        }
