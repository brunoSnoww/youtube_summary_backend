...
pub struct DownloadConfig {
    ...
    pub chosen_format: youtube::VideoQualityAndFormatPreferences,
    /// Whether the downloaded files have to be audio-only/video-only/normal video
    pub media_selected: youtube::MediaSelection,
...
impl DownloadConfig {
    ...
    fn choose_output_path(&self, command: &mut process::Command) {
        ...
        command.arg({
            ...
            if self.download_target == analyzer::DownloadOption::YtPlaylist {
                ...
                #[cfg(not(target_os = "windows"))]
                path_and_scheme.push_str("/audio.%(ext)s");
            }
            path_and_scheme
        });
    }
    ...
    fn choose_format(&self, command: &mut process::Command, format_id: &str) {
        match self.media_selected {
            youtube::MediaSelection::FullVideo => match &self.chosen_format {
            ...
            youtube::MediaSelection::AudioOnly => match &self.chosen_format {
                youtube::VideoQualityAndFormatPreferences::BestQuality => {
            ...
            youtube::MediaSelection::VideoOnly => match &self.chosen_format {
                youtube::VideoQualityAndFormatPreferences::BestQuality => {
