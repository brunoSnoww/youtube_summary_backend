...
mod format {
    ...
    use crate::yt_audio::error::{
        BEST_QUALITY_PROMPT_PLAYLIST, CONVERT_FORMAT_PROMPT_AUDIO,
        CONVERT_FORMAT_PROMPT_VIDEO_PLAYLIST, FFMPEG_UNAVAILABLE_WARNING,
        SMALLEST_QUALITY_PROMPT_PLAYLIST, YT_FORMAT_PROMPT_PLAYLIST,
    };
    ...
    fn get_format_from_yt(
        ...
        if ytdl_formats.stdout.is_empty() {
            return Err(BlobdlError::JsonSerializationError);
        }
    ...
    fn get_common_formats(
        json_formats: process::Output,
    ) -> BlobResult<(Vec<String>, FormatsLibrary)> {
